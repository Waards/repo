...
import { type PeerDot, type SignalMsg, type PulseEventData } from "@/lib/types";
...
export default function Home() {
  ...
  const [pulses, setPulses] = useState<PulseEventData[]>([]);
  ...
  return (
    <main className="fixed inset-0 overflow-hidden">
      ...
      {notice && (
        <div className="toast fixed left-1/2 top-20 z-50 -translate-x-1/2 rounded-full bg-zinc-800/90 px-5 py-2.5 text-sm text-zinc-100 shadow-lg shadow-black/30 backdrop-blur">
          {notice}
        </div>
      )}

      {conn.kind === "requesting" && (
        <div className="fixed left-1/2 top-20 z-50 flex -translate-x-1/2 items-center gap-3 rounded-full bg-zinc-800/90 px-5 py-2.5 text-sm text-zinc-100 shadow-lg shadow-black/30 backdrop-blur animate-fade-in">
          <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-emerald-400 border-t-transparent" />
          <span>Requesting connection&hellip;</span>
          <button
            onClick={cancelRequest}
            className="ml-2 rounded-full bg-zinc-700 px-3 py-1 text-xs transition hover:bg-zinc-600 active:scale-95"
          >
            Cancel
          </button>
        </div>
      ...
      {video === "requesting" && (
        <div className="fixed bottom-24 left-1/2 z-50 -translate-x-1/2 rounded-full bg-zinc-800/90 px-5 py-2.5 text-sm text-zinc-100 shadow-lg shadow-black/30 backdrop-blur animate-fade-in">
          <span className="inline-block h-3 w-3 animate-spin rounded-full border-2 border-emerald-400 border-t-transparent mr-2" />
          Waiting for stranger to accept video&hellip;
        </div>
      )}
